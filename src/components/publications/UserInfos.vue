...
// Utiliser storeToRefs pour obtenir une référence réactive à l'utilisateur du store.
// Cela garantit que le composant se mettra à jour lorsque les données de l'utilisateur changeront dans le store.
...
const editForm = reactive({
  ...
  telephone: '',
  biographie: '',
  photo: null, // Pour la prévisualisation de la photo
});

// S'assurer que les données de l'utilisateur sont chargées au montage du composant
onMounted(() => {
...
watch(user, (newUser) => {
  if (newUser && newUser.id) {
    // Si nous ne sommes pas en mode édition, mettez à jour le formulaire pour refléter les nouvelles données.
    if (!isEditing.value) {
...
// La fonction prend maintenant les données utilisateur en argument pour plus de clarté
...
function formatDate(dateString) {
  ...
  // Retourne la date au format YYYY-MM-DD pour l'input type="date"
...
function getCurrentPhoto() {
  // Affiche la photo prévisualisée si elle existe, sinon la photo de l'utilisateur
  if (editForm.photo) {
    return editForm.photo;
  }
  // Utilise l'utilisateur réactif du store
  return formatBase64Image(user.value.photo);
}

// Gestion de l'édition
function startEditing() {
  // Initialise le formulaire avec les données actuelles de l'utilisateur du store
...
function cancelEditing() {
  ...
  editForm.photo = null;
  // Réinitialiser le formulaire aux données originales de l'utilisateur en cas d'annulation
  if (user.value) {
    initializeForm(user.value);
  }
}
...
async function handleFileChange(event) {
  ...
  if (file && file.type.startsWith('image/')) {
    ...
    try {
      ...
      // Recharger pour afficher la nouvelle photo mise à jour par le backend
      // Cela mettra à jour le 'user' réactif du store, et l'interface suivra.
...
async function deletePhoto() {
  ...
  try {
    ...
    // Recharger les données pour que la photo disparaisse
    await userStore.fetchCurrentUser();
...
async function saveInfoChanges() {
  ...
  try {
    ...
    // Recharger les données utilisateur après la mise à jour pour le rafraîchissement
    // Cela mettra à jour le 'user' réactif du store, et l'interface suivra.
    await userStore.fetchCurrentUser(); 
...
<template>
  <div v-if="user && user.id" class="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div v-if="isEditing" class="absolute inset-0 flex items-center justify-center bg-black/50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              <div class="flex space-x-1">
                <button
                  ...
                  class="bg-white/90 hover:bg-white text-gray-800 p-1.5 rounded-full transition-colors"
                ...
                <button
                  ...
                  class="bg-red-500/90 hover:bg-red-500 text-white p-1.5 rounded-full transition-colors"
